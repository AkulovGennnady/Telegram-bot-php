...
class MyComplInvestm
{
	...
	{		
		...
		require __DIR__ . '/../Locale/MenuComm/MyComplInvestm/'.$lang.'.php';	 

		$deposar = $this -> getComplDepo();
		...
		if ($deposar){
			//Your NON active investments:				
			$sendMessage->text = "{$header}%0A";
			$TelegramMain -> performApiRequest($sendMessage);
			//each NON active
				foreach ($deposar as $depo)
				{	
					$depostr = "%0A{$s0}{$depo['id']}%0A{$s1}{$depo['name']}%0A{$s2}{$depo['amount']} USD%0A{$s3}{$depo['paym_sys']}%0A{$s4}".gmdate("d-M-Y H:i", $depo['time'])."%0A{$s5}{$depo['earned']} / ".$depo['amount']*$depo['pers']*$depo['numofpaym']." USD%0A{$s6}{$depo['p_num']} / {$depo['numofpaym']}%0A{$s7}".gmdate("d-M-Y H:i", $depo['lpt'])."%0A";
					//new one depo
